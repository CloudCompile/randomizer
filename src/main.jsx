...
function loadState() {
  try {
    ...
    const classes = saved.classes.map(item => ({ id: item.id || uid(), name: String(item.name || 'Untitled class'), students: Array.isArray(item.students) ? item.students.map(String).map(name => name.trim()).filter(Boolean) : [], groupCount: Number.isInteger(item.groupCount) && item.groupCount > 0 ? item.groupCount : 3 }));
...
function App() {
  ...
  const randomize = () => {
    ...
    setNotice(''); setResultAnimating(true); setState(prev => ({ ...prev, results: { classId: current.id, groups } })); window.setTimeout(revealResults, 100); window.setTimeout(() => setResultAnimating(false), 4200);
  };
